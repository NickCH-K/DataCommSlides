...
There are many more elements to this, but I will cover two tools: `pivot` and `join`
...
- Think about how we want the data to look - one row per (keys) category and month, and a column for the actual sales value
...
Pivot `data1` to one observation per month per category, then join with `data2`, maintaining both categories
...
- You can include some special characters, notably `'\n'` for line breaks.
- Pasting together annotations and table titles is a great idea!

## stringr

- Just like **forcats** for categories and **lubridate** for dates, **stringr** (in the **tidyverse** as well) makes working with strings as easy as possible
...
Then, create `income_num` equal to the first number: 
...
- Handy for processing many files, perhaps use `list.files()` to make a list of cSV files to read in, and then `map_df(read_csv)`!
...
- Saving data in R format: `save()` saves many objects, which are all put back in the environment with `load()`. 
- Alternately, `saveRDS()` which saves a single `data.frame()` or `tibble()` in compressed format, loadable with `df <- readRDS()`. Makes more sense coding-wise, but can't be opened in, say, Tableau, and is less standard and more confusing
- Saving data for sharing: `write_csv()` makes a CSV. Yay!
- If you wanna get fancy, `arrow::write_parquet()`/`arrow::read_parquet()`
...
1. Save the result as a CSV with `write_csv()`
...
1:5 %>% map_df(add_to_mtcars) %>% write_csv('myfile.csv')
```
